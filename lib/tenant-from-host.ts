...
export function getCurrentSubdomain(): string {
  ...
  if (hostname === "localhost" || hostname === "127.0.0.1") return "bidder"
  return hostname.split(".").length > 2 ? hostname.split(".")[0] : "bidder"
}
...
export function tenantFromHost(host: string): string {
  ...
  return parts.length > 2 ? parts[0] : (process.env.DEFAULT_TENANT ?? "bidder")
}
