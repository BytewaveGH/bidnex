...
import VendorOtpForm from './otp-form'
        ...
        <main className="min-h-screen">
            <section className="flex min-h-screen gap-40">
                <div className="hidden md:block md:w-1/2 relative">
                    <div className="h-svh py-4 mr-8 relative">
                        <div className="h-full w-full relative mx-4 rounded-xl overflow-hidden">
                            <Image src={authImg} layout="fill" objectFit="cover" alt="Vendor signup" className="rounded-xl" />
                ...
                <div className="w-full md:w-1/2 flex items-center justify-start min-h-screen">
                    {page === 'signup' ? (
                        <VendorSignupForm onChangePage={handleSignupSuccess} />
                    ) : (
                        <VendorOtpForm
                            phone={registeredPhone}
                            onChangePage={() => router.push('/vendor/auth/login')}
                        />
                    )}
                </div>
            </section>
