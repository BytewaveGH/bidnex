  ...
  if (isLoading && !data) {
    return (
      <div className="flex flex-col items-center justify-center w-full">
        <h2 className="text-2xl sm:text-3xl font-bold w-full mb-8">Related Products</h2>
      ...
      <div className="flex w-full items-start justify-between mb-8">
        ...
        <div className="flex items-center justify-center gap-2">
          <ButtonTemplate
            title={
              <ArrowLeft
                className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${canScrollLeft ? "text-[#2A3239]" : "text-[#98A2B3]"}`}
              />
            }
            className={`bg-white text-black hover:bg-white h-8 w-8 sm:h-10 sm:w-10 border rounded-full p-0 cursor-pointer ${canScrollLeft ? "border-[#2A3239]" : "border-[#98A2B3]"}`}
            onClick={scrollLeft}
          />
          <ButtonTemplate
            title={
              <ArrowRight
                className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${canScrollRight ? "text-[#2A3239]" : "text-[#98A2B3]"}`}
              />
            }
            className={`bg-white text-black hover:bg-white h-8 w-8 sm:h-10 sm:w-10 border rounded-full p-0 cursor-pointer ${canScrollRight ? "border-[#2A3239]" : "border-[#98A2B3]"}`}
            onClick={scrollRight}
          />
        </div>
      </div>
